      ...
      <div class="mb-3 form-group col-12">
        ...
        >
          <div class="uploaddetails">
            ...
            <input type="file" ref="fileInputMultiple" @change="handleFileSelect" style="display: none" multiple />
          </div>
        </div>
      </div>

      <div class="col-12 file-listing-container h-24 flex gap-4" v-if="imagePreviews?.length">
        <div class="file-listing h-24" v-for="(image, index) in imagePreviews" :key="index">
          <img class="preview h-24" :src="image" />
          <div class="remove-container">
            <i class="ti ti-trash fs-3 delete" @click="removeImage(index)"></i>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      imagePreviews: this.galleries?.map((gallery) => gallery.imgPath) || [],
      isDrag: false,
      isUploading: false,
  ...
  watch: {
    galleries(newVal) {
      this.imagePreviews = newVal?.map((gallery) => gallery.imgPath);
    },
  },
  mounted() {
  ...
  methods: {
    ...
    ondragleave() {
      this.text = "Drop your files here Or Browse";
      this.isDrag = false;
    },
